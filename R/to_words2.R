...
#' to_words2("-542687982546720143")
#' @export


to_words2 = function(x){
  if (is.character(x)) {
    ...
    if (!grepl("\\D", x)){
      y = x
      if (nchar(y) > 60){
          stop("That there is a big number. I ran out of words.")
      }
    }else{
  ...
  cleaned_text = paste0(
          negative_text,
          clean_text2(paste(raw_text, collapse = " "))
  )
  return(cleaned_text)
}

clean_text2 = function(n){
